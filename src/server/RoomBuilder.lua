...
local LIGHT_BRICK_WALL_TEXTURE_ID = "rbxassetid://12813020091"
local ATOMIC_COLORS = {
...
local function makeShell(roomFolder)
	...
	local backWall = createPart(roomFolder, "BackWall", Vector3.new(width, height, 1), CFrame.new(0, height / 2, -depth / 2), wallColor, Enum.Material.SmoothPlastic)
	local leftWall = createPart(roomFolder, "LeftWall", Vector3.new(1, height, depth), CFrame.new(-width / 2, height / 2, 0), wallColor, Enum.Material.SmoothPlastic)
	local rightWall = createPart(roomFolder, "RightWall", Vector3.new(1, height, depth), CFrame.new(width / 2, height / 2, 0), wallColor, Enum.Material.SmoothPlastic)
...
local function makeInfirmaryRoom(roomFolder)
	...
	local infirmaryClockPosition = origin + Vector3.new(width / 2 - 0.62, 9.2, -6.5)
	local infirmaryClockCFrame = CFrame.lookAt(
		infirmaryClockPosition,
		Vector3.new(origin.X, infirmaryClockPosition.Y, origin.Z)
	)
	createNoTouchClock(
		room,
		"InfirmaryWallClock",
	...
	local monitorLookTowardRoom = (Vector3.new(origin.X, monitorPosition.Y, origin.Z) - monitorPosition).Unit
	local monitor = createPart(room, "InfirmaryMonitor", Vector3.new(5.4, 3.2, 0.34), CFrame.lookAt(monitorPosition, monitorPosition + monitorLookTowardRoom), Color3.fromRGB(18, 30, 33), Enum.Material.Metal)
